 ...
 */
const fetchWithRetry = async (url: string, retries = 2, backoff = 1000): Promise<Response> => {
  try {
...
export const fetchTrending = async (type: 'movie' | 'tv' | 'all' = 'movie', page: number = 1): Promise<{ results: Movie[], totalPages: number }> => {
  ...
  try {
    const response = await fetchWithRetry(`${TMDB_BASE_URL}/trending/${type}/week?api_key=${TMDB_API_KEY}&page=${page}`);
...
export const fetchUpcomingMovies = async (page: number = 1): Promise<{ results: Movie[], totalPages: number }> => {
  ...
  try {
    const response = await fetchWithRetry(`${TMDB_BASE_URL}/discover/movie?api_key=${TMDB_API_KEY}&page=${page}&primary_release_year=2026&sort_by=popularity.desc&include_adult=false`);
    const data = await handleResponse(response, cacheKey);
    return {
      results: (data.results || []).map((m: any) => ({ ...m, media_type: 'movie' })),
      totalPages: data.total_pages || 1
...
export const fetchUpcomingTV = async (page: number = 1): Promise<{ results: Movie[], totalPages: number }> => {
  ...
  try {
    const response = await fetchWithRetry(`${TMDB_BASE_URL}/discover/tv?api_key=${TMDB_API_KEY}&page=${page}&first_air_date_year=2026&sort_by=popularity.desc&include_adult=false`);
    const data = await handleResponse(response, cacheKey);
    return {
...
export const fetchGenres = async (type: 'movie' | 'tv'): Promise<{ id: number, name: string }[]> => {
  ...
  try {
    const response = await fetchWithRetry(`${TMDB_BASE_URL}/genre/${type}/list?api_key=${TMDB_API_KEY}`);
    const data = await handleResponse(response, cacheKey);
    return data.genres || [];
  } catch (err) {
...
export const searchMedia = async (query: string, type: 'movie' | 'tv' | 'all', page: number = 1, year?: string): Promise<{ results: Movie[], totalPages: number }> => {
  try {
    if (type === 'all') {
      const response = await fetchWithRetry(`${TMDB_BASE_URL}/search/multi?api_key=${TMDB_API_KEY}&query=${encodeURIComponent(query)}&page=${page}`);
    ...
    const response = await fetchWithRetry(`${TMDB_BASE_URL}/${endpoint}?api_key=${TMDB_API_KEY}&query=${encodeURIComponent(query)}&page=${page}${year ? yearParam : ''}`);
    const data = await handleResponse(response);
...
export const fetchAnime = async (page: number = 1, subGenre?: number): Promise<{ results: Movie[], totalPages: number }> => {
  ...
  try {
    ...
    console.error("fetchAnime failed critically:", err);
    return { results: [], totalPages: 0 };
  }
};
...
export const getDetails = async (id: number, type: 'movie' | 'tv'): Promise<Movie> => {
  ...
  const response = await fetchWithRetry(`${TMDB_BASE_URL}/${type}/${id}?api_key=${TMDB_API_KEY}&append_to_response=external_ids,videos,credits`);
  const data = await handleResponse(response, cacheKey);
...
export const fetchSimilar = async (id: number, type: 'movie' | 'tv'): Promise<Movie[]> => {
  ...
  try {
    const response = await fetchWithRetry(`${TMDB_BASE_URL}/${type}/${id}/similar?api_key=${TMDB_API_KEY}`);
    const data = await handleResponse(response, cacheKey);
...
export const getSeasonEpisodes = async (id: number, season: number): Promise<Episode[]> => {
  ...
  try {
    const response = await fetchWithRetry(`${TMDB_BASE_URL}/tv/${id}/season/${season}?api_key=${TMDB_API_KEY}`);
    const data = await handleResponse(response, cacheKey);
...
export const findByTitle = async (title: string): Promise<Movie | null> => {
  try {
    const response = await fetchWithRetry(`${TMDB_BASE_URL}/search/multi?api_key=${TMDB_API_KEY}&query=${encodeURIComponent(title)}`);
    const data = await handleResponse(response);
