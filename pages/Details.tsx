...
const Details: React.FC = () => {
  ...
  // Sleep Timer States (Background only)
  ...
  // Sleep Timer logic (Silent background operation)
  useEffect(() => {
      ...
      sleepIntervalRef.current = window.setInterval(() => {
        setSleepTimeRemaining(prev => {
          if (prev !== null && prev <= 1) {
            ...
            setAutoPreviewActive(false);
            if (sleepIntervalRef.current) window.clearInterval(sleepIntervalRef.current);
            // ZEN FEATURE: Automatically return to home screen when timer expires
            navigate('/');
            return null;
          }
    ...
    <div className="min-h-screen pb-20 bg-[#040404]">
      {isPlaying ? (
        <div className="relative w-full aspect-video max-w-7xl mx-auto md:rounded-xl mt-0 md:mt-8 bg-black shadow-2xl overflow-hidden group/player">
          ...
          {/* Sleep Timer Controls - Silent Mode */}
          <div className="absolute top-4 right-4 z-50 flex flex-col items-end gap-2">
            <div className="relative">
              <button 
                ...
                title={sleepTimeRemaining !== null ? "Sleep Timer Active" : "Set Sleep Timer"}
              >
                ...
                <div className="absolute top-full right-0 mt-2 w-48 bg-[#0c0c0c]/90 backdrop-blur-xl border border-white/10 rounded-2xl p-2 shadow-2xl animate-in zoom-in-95 fade-in duration-200">
                  ...
                  {[1, 15, 30, 45, 60].map(mins => (
                    <button 
                      ...
                      className="w-full text-left px-4 py-2.5 rounded-xl hover:bg-[#1ce783] hover:text-black transition-all text-xs font-black uppercase tracking-widest text-gray-300"
                    >
                  ...
                  <button 
                    ...
                    className="w-full text-left px-4 py-2.5 rounded-xl hover:bg-red-500/20 text-red-500 transition-all text-xs font-black uppercase tracking-widest mt-1 border-t border-white/5 pt-2"
                  >
                    Turn Off
                  </button>
                </div>
            ...
            <>
              ...
              <div className="absolute bottom-0 left-0 p-6 md:p-16 w-full max-w-5xl z-20">
                <div className="flex items-center gap-3 md:gap-4 mb-4 animate-in slide-in-from-left-4 duration-700">
                  <span className="text-[#1ce783] text-sm md:text-xl font-black flex items-center gap-1 drop-shadow-[0_0_10px_rgba(28,231,131,0.5)]">
                ...
                <div className="flex flex-wrap gap-3 md:gap-4 animate-in slide-in-from-bottom-4 duration-1000">
                  <button onClick={() => playMedia()} className="flex-1 md:flex-none bg-white text-black px-8 md:px-10 py-3 md:py-4 rounded-sm font-black text-xs md:text-lg hover:bg-[#1ce783] transition-all transform active:scale-95 flex items-center justify-center gap-2 md:gap-3 uppercase tracking-widest shadow-2xl">Watch Now</button>
              ...
              {trailer && (
                <div className="absolute top-6 right-6 md:top-24 md:right-16 z-30 flex flex-col items-center gap-2 md:gap-3">
                  <button 
                    ...
                    className={`relative w-12 h-12 md:w-20 md:h-20 rounded-full flex items-center justify-center transition-all duration-500 overflow-hidden shadow-2xl group/toggle ${
                      autoPreviewActive 
                  ...
                  >
                    ...
                    <div className={`relative z-10 transition-transform duration-1000 ${autoPreviewActive ? 'animate-[spin_4s_linear_infinite]' : ''}`}>
                      {autoPreviewActive ? (
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="w-6 h-6 md:w-8 md:h-8">
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-16 py-10 md:py-16">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-12 md:gap-16">
          <div className="lg:col-span-2 space-y-10 md:space-y-12">
            <section>
              <div className="flex flex-wrap items-center gap-3 md:gap-4 mb-4 md:mb-4">
            ...
            {type === 'tv' && media && (
              <section className="space-y-6 md:space-y-8">
                <div className="flex items-center justify-between border-b border-white/10 pb-4 md:pb-4">
                  ...
                  <div className="flex items-center gap-3 md:gap-4">
                    ...
                    <select 
                      ...
                      className="bg-black border border-white/20 rounded px-3 md:px-4 py-1.5 md:py-1.5 text-[10px] md:text-xs font-black uppercase outline-none focus:border-[#1ce783] transition-colors cursor-pointer text-white"
                    >
                      {[...Array(media.number_of_seasons)].map((_, i) => (<option key={i} value={i + 1}>Season {i + 1}</option>))}
                    </select>
                  </div>
                </div>
                <div className="flex flex-nowrap overflow-x-auto gap-4 md:gap-4 pb-4 custom-scrollbar scroll-smooth">
                  {episodes.map((ep) => (
                    <div 
                      key={ep.id} onClick={() => playMedia(ep.episode_number)}
                      className={`min-w-[240px] md:min-w-[320px] max-w-[240px] md:max-w-[320px] flex flex-col gap-3 md:gap-3 p-3 md:p-3 rounded-2xl transition-all cursor-pointer group shrink-0 ${currentEpisode === ep.episode_number && isPlaying ? 'bg-[#1ce783]/10 border border-[#1ce783]/40' : 'bg-white/5 hover:bg-white/10 border border-white/5'}`}
                    >
                      ...
                      <div className="px-1">
                        <div className="flex items-center justify-between mb-2">
                          <h4 className="text-[11px] md:text-xs font-black text-white uppercase tracking-tight truncate">{ep.name}</h4>
                          <span className="text-[10px] md:text-[10px] text-[#1ce783] font-black">EP {ep.episode_number}</span>
                        </div>
                        <p className="text-[10px] md:text-[10px] text-gray-500 line-clamp-2 leading-relaxed">{ep.overview || "No description available."}</p>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <section className="space-y-8 md:space-y-8 pt-12">
              <div className="border-b border-white/10 pb-4 md:pb-4">
                  <h2 className="text-xl md:text-2xl font-black uppercase italic tracking-tighter">Recommended</h2>
              </div>
              {loadingSimilar ? (
                  <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-5 gap-4 md:gap-4">
                    {[1, 2, 3, 4, 5, 6].map(i => (<div key={i} className="aspect-[2/3] bg-white/5 animate-pulse rounded-2xl" />))}
                  </div>
              ) : (
                  <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-4 md:gap-4">
          ...
          <div className="lg:col-span-1 space-y-10 md:space-y-10">
            <div className="bg-white/5 border border-white/10 rounded-3xl p-8 md:p-8 space-y-10 backdrop-blur-xl">
                <div>
                  <p className="text-[10px] font-black text-gray-500 uppercase tracking-[0.3em] mb-4">Original Language</p>
                  <p className="text-white font-black uppercase italic text-xl">{media?.original_language === 'en' ? 'English' : media?.original_language}</p>
                </div>
                <div>
                  <p className="text-[10px] font-black text-gray-500 uppercase tracking-[0.3em] mb-4">Popularity Index</p>
                  <div className="flex items-center gap-4">
                      <span className="text-[#1ce783] font-black text-4xl italic">{media?.popularity.toFixed(0)}</span>
                  </div>
                </div>
            </div>
          </div>
