...
const Details: React.FC = () => {
  ...
  return (
    <div className="min-h-screen pb-20 bg-[#040404]">
      <div className="relative h-[65vh] md:h-[80vh] w-full bg-black overflow-hidden shadow-2xl">
        {loading || !media ? (
          ...
          <>
            ...
            <div className="absolute top-24 md:top-32 left-0 p-6 md:p-16 w-full max-w-5xl z-20 space-y-6">
              <div className="flex items-center gap-2 text-[#1ce783] text-[10px] md:text-sm font-black uppercase tracking-widest animate-in slide-in-from-left-4 duration-700">
                <span>★ {media.vote_average.toFixed(1)}</span>
                <span className="text-white/20">•</span>
                <span className="text-white">{releaseYear}</span>
              </div>

              <h1 className="text-3xl md:text-7xl font-black uppercase italic tracking-tighter leading-none drop-shadow-2xl animate-in slide-in-from-left-6 duration-700 line-clamp-2">
                {media.title || media.name}
              </h1>

              <div className="flex items-center gap-2 animate-in slide-in-from-bottom-4 duration-1000 w-full md:w-auto">
                <button 
                  onClick={() => playMedia()} 
                  className="flex-1 md:flex-none bg-white text-black px-8 md:px-12 py-2 md:py-2.5 rounded-sm font-black text-[10px] md:text-sm uppercase tracking-widest hover:bg-[#1ce783] transition-all transform active:scale-95 shadow-2xl whitespace-nowrap text-center"
                >
                  {lastWatched ? 'Continue' : 'Watch Now'}
                </button>
                <button 
                  onClick={() => toggleWatchlist(media)} 
                  className="flex-1 md:flex-none bg-white/10 backdrop-blur-md text-white px-8 md:px-12 py-2 md:py-2.5 rounded-sm font-black text-[10px] md:text-sm uppercase tracking-widest border border-white/10 hover:bg-white/20 transition-all whitespace-nowrap text-center"
                >
                  {inList ? 'Saved' : 'Add To List'}
                </button>
              </div>

              <div className="flex flex-wrap items-center gap-3 md:gap-4 pt-4 animate-in slide-in-from-bottom-6 duration-1000">
                <span className="text-[#1ce783] font-black uppercase text-[9px] md:text-xs tracking-widest">IMDb Score ★ {media.vote_average.toFixed(1)}</span>
                <span className="text-gray-400 font-black text-[9px] md:text-xs">{releaseYear}</span>
                <div className="flex gap-2">
                  {media.genres?.slice(0, 2).map(g => (
                    <span key={g.id} className="text-black font-black text-[8px] md:text-[9px] uppercase tracking-widest bg-[#1ce783] px-2.5 py-1 rounded-sm">{g.name}</span>
                  ))}
                </div>
              </div>

              <p className="text-gray-300 text-[10px] md:text-base leading-relaxed max-w-2xl font-medium animate-in slide-in-from-bottom-8 duration-1000 line-clamp-3 md:line-clamp-none">
                {media.overview}
              </p>
            </div>

            {trailer && (
              <div className="absolute top-20 right-6 md:top-32 md:right-16 z-30 flex flex-col items-center gap-2 md:gap-3">
                <button 
